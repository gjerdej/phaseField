                                            ...
                                            [[maybe_unused]] Vector<double>    &vector_IC)
{
  ...
  if (index == 0){
    ...
    scalar_IC = 0.5*(1.0+std::tanh(-(p[1]-userInputs.domain_size[1]/2.0)/(int_width/2.0)));
  }
  ...
  if (index == 2){
    scalar_IC = 0.5 * (1.0 + std::tanh(-(p[1] - userInputs.domain_size[1] / 2.0) / (int_width / 2.0))) + 1.0e-6;
    // scalar_IC = 0.5 * (1.0 + std::tanh(-(p[1] - userInputs.domain_size[1] / 2.0) / (int_width / 2.0))) + 1.0e-6;
  }
